...
class DaskMLMatcher(Matcher):
    ...
    def predict(self, x=None, table=None, exclude_attrs=None, target_attr=None,
        ...
        if x is not None:
            ...
            n_procs = gh.get_num_procs(n_jobs, 1)
            if n_procs ==  1 or inplace == False or append == False:
                ...
                for i in range(len(splitted_tables)):
                    partial_result = delayed(self._predict)(x, splitted_tables[i],
                                                            exclude_attrs, target_attr,
                                                            append, return_probs,
                                                            probs_attr, inplace)
                    ...
                    if show_progress:
                        with ProgressBar():
                            predicted_results = predicted_results.compute(
                                scheduler="processes", num_workers=n_procs)
                    else:
                        predicted_results = predicted_results.compute(
                            scheduler="processes", num_workers=n_procs)

                return pd.concat(predicted_results)
